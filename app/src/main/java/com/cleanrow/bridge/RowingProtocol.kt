...
/**
 * Rowing machine serial protocol implementation.
 * Handles packet parsing and command building for Sportstech S-Row.
 * 
 * Packet format:
 * [0]     0x49 'I'    — header byte 1
 * [1]     0x54 'T'    — header byte 2
 * [2]     category    — command group (0x01=drive, 0x02=LED, 0x03=counters)
 * [3]     command     — specific command within group
 * [4]     N           — number of data bytes that follow
 * [5..4+N] data       — N bytes of payload
 * [5+N]   checksum    — XOR of bytes [2]..[4+N]
 * [6+N]   0x61 'a'    — end byte
 */
object RowingProtocol {
    ...
    /**
     * Set LED RGB color (0-255 per channel).
     * Note: Packet order is R, B, G (not R, G, B!)
     */
